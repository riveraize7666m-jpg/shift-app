...
config = {
    ...
    'cookie': {
        'expiry_days': 30,
        'key': 'shift_manager_signature_key',
        'name': 'shift_manager_cookie'
    }
}
...
# 1. アプリの設定 & デザイン
# ==========================================
st.set_page_config(page_title="Shift Manager Pro v50", layout="wide", page_icon="🗓️")
...
if auth_status is False:
    ...
    st.warning('利用にはログインが必要です。')
    st.info("【テスト用】ユーザー名: admin / パスワード: abc123")
    
    # マネタイズ導線
    st.markdown("---")
    st.subheader("💎 新規利用のお申し込み")
    st.write("アカウント登録後の案内となります。")
    paypal_url = "https://www.paypal.com/jp/home" 
    st.link_button("PayPalで申し込む (月額 ¥1,000)", paypal_url)
    st.caption("※確認後、メールで案内を送付します。")
...
if auth_status:
    ...
    st.title("🗓️ Shift Manager Pro v50")
    ...
    with st.sidebar:
        ...
        if st.session_state.staff_list:
            del_name = st.selectbox("削除選択", [s["name"] for s in st.session_state.staff_list], key="del_select")
            if st.button("削除実行"):
    ...
    for idx, staff in enumerate(st.session_state.staff_list):
        ...
        with st.sidebar.expander(f"{name}", expanded=False):
            ...
            f1, f2, f3 = "", "", ""
            if st.checkbox("年始固定", key=f"open_fix_{name}"):
    ...
    def solve_shift(staff_data):
        my_bar = st.progress(0, text="作成中...")
        best_schedule = None
        best_score = -999999
        max_attempts = 1500 
        work_limits = {}
        for s in staff_data:
            if s["type"] != 0: work_limits[s["name"]] = 99 
            else:
                extra = len(s["refresh_days"]) + len(s["paid_leave_days"])
                work_limits[s["name"]] = DAYS - (TARGET_OFF_DAYS + extra)
        ...
        for attempt in range(max_attempts):
            ...
            for s in staff_data:
                ...
                for stype_str, r_list in [("早","req_early"), ("遅","req_late"), ("日","req_day")]:
                    for d in s[r_list]:
                        if 0 < d <= DAYS and not schedule[nm][d-1]: schedule[nm][d-1] = stype_str
                if s["type"] == 1:
                    for d in range(DAYS):
            ...
            # Phase 2 & 3: 割り当て
            days_idx = list(range(DAYS))
            random.shuffle(days_idx)
            for d in days_idx:
                if any(schedule[x["name"]][d] == "夜" for x in staff_data): continue
            ...
            for _ in range(5):
                ...
                short = [d for d, c in d_counts.items() if c < 3]
                surp = [d for d, c in d_counts.items() if c > 3]
                ...
                for sd in short:
                    for ud in surp:
                        for s in regs:
                            nm = s["name"]
                            if schedule[nm][ud] in ["早","日","遅"] and schedule[nm][sd] == "◎":
                                if check_rules(nm, sd, schedule, schedule[nm][ud]):
                                    schedule[nm][sd] = schedule[nm][ud]; schedule[nm][ud] = "◎"
                                    swapped = True; break
                        if swapped: break
                    if swapped: break
            ...
            for s in staff_data:
                if s["type"] == 0:
                    ...
                    sc -= abs(cnt - TARGET_OFF_DAYS) * 100
            for d in range(DAYS):
            ...
            if sc > best_score:
                best_score = sc; best_schedule = copy.deepcopy(schedule)
            if sc > -50: break
    ...
    if st.session_state.get('shift_success'):
        ...
        _, c_days = calendar.monthrange(st.session_state.current_year, st.session_state.current_month)
        w_ja = ["月","火","水","木","金","土","日"]
        cols = [f"{d+1}({w_ja[datetime.date(st.session_state.current_year, st.session_state.current_month, d+1).weekday()]})" for d in range(c_days)]
        df_disp.columns = cols + ['夜勤', '公休']

        def color_shift(val):
            v = str(val); color = 'black'; bg = ''
            if v == '◎ ': bg = '#15803d'; color = 'white'
            elif v == '◎': bg = '#dcfce7'
            elif v == '有': bg = '#fbcfe8'
            elif v == 'リ休': bg = '#ffedd5'
            elif v == '夜': bg = '#1E3A8A'; color = 'white'
            elif v == '・': bg = '#BFDBFE'
            elif v == '早': bg = '#FDE047'
            elif v == '遅': bg = '#FDBA74'
            elif v == '日': bg = '#FFFFFF'
                ...
                return f'background-color: {bg}; color: black; border: 1px solid #ddd;'
