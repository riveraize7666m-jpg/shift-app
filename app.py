...
def show_help_dialog():
    ...
    **Step 1. 設定ファイルの読み込み（任意）**
    ...
    > - **スタッフ名のみ**：新しい月のシフト作成時に便利
    > - **全ての設定**：作業途中の再開や、やり直し時に便利
    ...
    **Step 5. シフト作成**
    ...
    > 
    > ⚠️ **ボタンを押すたびにシフトを再作成します。**
    > 「確認ポイント」が一番少ない結果が出るまで、何度か再作成してください。
    
    **Step 6. 設定ファイルの保存（重要）**
    > 作業が終わったら、必ず設定ファイルを保存してください。
    > - 次月の作業用に「スタッフ名のみ」を保存
    > - 念のため「全ての設定」も保存しておくと安心です
    
    ---
    
    ### ⚙️ シフト自動作成のルール
    
    このツールは以下のルールに基づいてシフトを自動作成します：
    
    **勤務日数のルール**
    - 連続勤務は**最大5日まで**（6連勤以上は禁止）
    - 夜勤の翌日は必ず「・（明け）」、その翌日は「◎（公休）」
    
    **シフト順序のルール（逆行禁止）**
    - 早番 → 日勤 → 遅番 → 夜勤 の順序で進行
    - 逆行（例：日勤→早番、遅番→日勤）は禁止
    - ただし、公休を挟めばリセットされます
    ...
    **2種類の保存形式**
    ...
    | 形式 | 内容 | 用途 |
    |:---|:---|:---|
    | スタッフ名のみ | 名前と属性のみ | 翌月のシフト作成開始時 |
    | 全ての設定 | 希望休・夜勤目標など全て | 作業の中断・再開、やり直し |
    
    **「スタッフ名のみ」の活用**
    > 毎月のシフト作成時、スタッフを一から登録する手間を省けます。
    > 前月に保存した「スタッフ名のみ」ファイルを読み込むだけでOK。
    
    **「全ての設定」の活用**
    > - 1日で作業が終わらなかった場合の続き
    > - シフト作成後に手動で調整したが、最初からやり直したい場合
    > - 念のためのバックアップ
    """)
...
# 設定ファイル読み込み
st.sidebar.markdown('<p style="color: #94a3b8; font-size: 0.8rem; margin-bottom: 0.5rem;">設定ファイルを読み込み</p>', unsafe_allow_html=True)
...
# 保存ボタン（2種類）
st.sidebar.markdown('<p style="color: #94a3b8; font-size: 0.8rem; margin: 0.75rem 0 0.5rem 0;">設定ファイルを保存</p>', unsafe_allow_html=True)
...
for s in st.session_state.staff_list:
    ...
    for k in keys:
        if k in st.session_state: export_full[k] = st.session_state[k]

col_save1, col_save2 = st.sidebar.columns(2)
with col_save1:
    st.download_button(
        "👥 スタッフ名のみ", 
        json.dumps(export_staff_only, indent=2, ensure_ascii=False), 
        'staff_list.json', 
        'application/json', 
...
with col_save2:
    st.download_button(
        "📋 全ての設定", 
        json.dumps(export_full, indent=2, ensure_ascii=False), 
        'shift_settings_full.json', 
        'application/json', 
        use_container_width=True,
        help="作業の中断・再開用"
    )
