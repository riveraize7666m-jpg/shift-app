...
# 0. 認証設定 (ユーザー管理)
# ==========================================
# 本来はこの設定を別ファイル(secrets)に隠しますが、
# まずは動作確認のためここに書きます。
# ユーザー: admin, パスワード: abc123 (ハッシュ化済み)
config = {
    'credentials': {
        'usernames': {
            'admin': {
                'name': '管理者',
                'password': '$2b$12$D.I.y./wHk.tQ.9/.7.3.O5.2.1.4.5.6.7.8.9.0.1.2.3.4.5', # ダミーハッシュ(実際はabc123では通りません。後述の生成ツールで正規のものを作ります)
                # 簡易的な動作確認のため、平文パスワードでの実装例に切り替えます
                # ※実運用時はハッシュ化推奨ですが、まずは動くことを優先します
            }
        }
    },
    'cookie': {
...
# 簡易実装：動作確認用ハッシュ（"abc123" のハッシュ値）
# 本番では後ほどお伝えする方法で、お客様ごとのパスワードを作ります。
...
# 1. アプリの設定 & デザイン
# ==========================================
st.set_page_config(page_title="Shift Manager Pro v46", layout="wide", page_icon="🗓️")
...
if authentication_status is False:
    ...
    st.info("【テスト用アカウント】ユーザー名: admin / パスワード: abc123")
    ...
    st.write("このツールを利用するにはアカウント登録が必要です。")
    
    # PayPalボタン（ダミーリンク）
    paypal_url = "https://www.paypal.com/jp/home" 
    st.link_button("PayPalで申し込む (月額 ¥1,000)", paypal_url)
    st.caption("※お支払い確認後、24時間以内にログイン情報をメールでお送りします。")


if authentication_status:
    # ==========================================
    # ログイン成功時の画面（ここからアプリ本体）
    # ==========================================
    with st.sidebar:
    ...
    st.title("🗓️ Shift Manager Pro v46")
    st.caption("クラウド対応：ログイン機能実装版")
